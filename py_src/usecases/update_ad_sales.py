...
class UpdateAdSalesUseCase:
    ...
    def execute_range(self, start: date, end: date) -> AdWriteResult:
        ...
        metrics_by_date = self._ads_repository.get_daily_metrics(start, end)
        self._guard_against_empty_report(metrics_by_date, start, end)
        filled_metrics_by_date = self._fill_missing_dates(metrics_by_date, start, end)
        return self._ad_sheet.write_ad_metrics(filled_metrics_by_date)

    @staticmethod
    def _guard_against_empty_report(
        metrics_by_date: MetricsByDate, start: date, end: date,
    ) -> None:
        if not any(metrics_by_date.values()):
    ...
    def _fill_missing_dates(
        ...
        return {day.isoformat(): metrics_by_date.get(day.isoformat(), {}) for day in all_days}
