...
- 直近1ヶ月に更新された在庫のみを取得します（startDateTimeパラメータを使用）
- SKU指定なしで在庫を取得するため、「売上/日」シートに登録されていない商品も含まれます
...
**逆の対応**になる。広告費は下げるのが good、商品価格は上げるのが good だから。

営業利益だけ見ても、それが入札を下げた結果なのか上げた結果なのかが分からない。
色を付けておくと、後から意図と結果を突き合わせられる。

- 入札の履歴は `marketar/ad/docs/report/bids.json`（広告レポートが毎日記録する）
- ターゲットと ASIN の対応は同じディレクトリの `ad-performance.html` から読む
- **Amazon の API は過去の入札を返さない。** 記録が始まる前（2026-08-29 以前）は色が付かない
- 1 つの商品で下げと上げが混ざる日は多い方を採る。同数なら色を付けない
