...
| # | ラベル | 中身 | 書くのは | 色 |
|---|---|---|---|---|
| 0 | （ASIN行） | その日の売上個数。ノートに価格 | `main.py daily` / `today` | うすい赤→青 |
| 1 | `営業利益` | **数式** `=IF(粗利益="","",粗利益-N(広告費))` | 粗利益を書くたびに入れ直す | 濃い赤←0円=白→濃い青 |
...
- **個数は商品ごとのスケール、営業利益は全商品で1つのスケール。** 個数は商品ごとに
  桁が違い（日次200個 と 数個）、1つに載せると後者がすべて同じ色になる。営業利益は
  0円を白に固定してあるので、金額そのものを商品間で比べられる
- **営業利益は 0円 = 白。** MIN を白にすると「最も赤字の日」が白になり、黒字か赤字かが
  色から読めない
