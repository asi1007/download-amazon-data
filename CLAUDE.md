...
| `ranks` | `UpdateCategoryRanksUseCase` | カテゴリ順位を「売上/日」の順位行へ書く（`--date` で日付指定）|
...
- `backfill_daily_sales.py` — 任意日付の「売上/日」バックフィル（複数日対応、`batch_update` でクォータ回避）
  ...
  - **日付ごとに全 ASIN を1秒間隔で叩く。** 1日あたり約80秒かかるので、1か月分で40分見ておく
  - **日付列が無い日を渡すと最新列の位置に挿入され、並び順が壊れる。** 既に列がある範囲だけを渡す
  - 「売上/日」の日付列構造は auto-memory `reference_sales_daily_sheet_structure.md` 参照
- `backfill_ad_sales.py <開始> <終了>` — 広告経由・広告費（31日ずつに分割して Ads API を叩く）
- `backfill_operating_profit.py [--days N]` — 営業利益の数式を入れ直す（既定は全列）
- `recolor_price_changes.py [--dry-run]` — 売上個数セルの文字色を、記録済みの価格ノートから塗り直す
...
日付ラベル（行1・行4）と総売上（**行2**）の `numberFormat` は、**書き込みのたびに `apply_column_formats()` で明示適用する**。列挿入時の隣接列からのフォーマット継承には依存しない（継承が効かず、シリアル値のまま表示される事故があった）。

**総売上（行2）はセルに円のまま書き、表示だけ千円単位にする**（`#,##0,"千円"`）。値を 1000 で割って書かないこと。他シートからの参照が円前提であり、Sheets の数値書式はカンマ1個で 1/1000 にスケールできるため、値を変える必要がない（万円単位は 10^4 で書式では表現できない）。

### 集計行は 行2=売上合計 / 行3=利益合計 / 行4=利益率（2026-09-10〜）

日付列は毎日増えて位置が動くため、**どちらの数字かは列の位置では分からない。上下の並びで見分ける。**
売上を上、利益をその下、利益率をさらに下に置く。

**行構成の正本は `shared/sales-data` の `layout.py`。** このリポジトリは行番号を持たない。

```python
SUMMARY_ROWS_IN_ORDER = ("総売上", "営業利益の合計", "利益率")   # ← 行を足すならここ
HEADER_ROW = DATE_LABEL_ROW + len(SUMMARY_ROWS_IN_ORDER) + 1    # = 5
```

- 2026-09-09 に売上と利益の上下を入れ替えた（それまでは行2が利益、行3が売上）
- 2026-09-10 に**利益率**（行4）を足し、ヘッダー行が 4 → **5** になった。既存の87列も移行済み
- **利益率は値を持たず `=IF(<列>$2=0,"",<列>$3/<列>$2)` で導く。** 売上合計と利益合計の
  どちらが後から更新されても追従する（営業利益が数式なのと同じ考え方）。売れていない日は
  総売上が0なので、そのまま割ると `#DIV/0!` が並ぶ
- **GAS 側（`updatePrice.js` / `CostDataReader.js`）は Python ライブラリを使えないので値を写している。**
  行を足したら**必ずここも直す**。忘れると `updatePrice()` が誤った行を掴んで Amazon に価格を PATCH する

### データ取得時刻は「目標販売数」列の行2に書く（v0.19.0〜）

当日列は `today` が毎時0分に上書きするため、値だけでは「いつ時点の数字か」が分からない。
`write_sales_nums` は**日付列ではなく**、位置の動かない「目標販売数」列の**行2**（`FETCH_TIME_ROW`、
売上合計と同じ行）へ書き込み時刻を **`HH:MM`（JST）** の文字列で書く。
**「取得」の文字は付けない**（2026-09-09〜。何の時刻かは位置で分かる）。
**日付も書かない**（バックフィルした列で日付と紛らわしくなるため、あえて時刻のみ）。
**過去日のバックフィルでは書かない**（今日の更新時刻が壊れる）。
...
`write_prices` は前日列のノートを `get_notes(grid_range=...)` で読み、当日価格と比べて**値下げなら赤字・値上げなら青字**（いずれも太字）にする。**セルの値（`worksheet.get`）を読んではいけない。** 価格は個数より必ず大きいため、全 ASIN が「値上げ」判定になり列全体が同じ色になる（GAS の `getNote()` を移植時に `.value` と取り違えて実際にそうなっていた。2026-08-12 修正）。

### 価格変動は背景色ではなく文字色で出す（2026-09-08〜）

**売上個数セルの背景は条件付き書式（個数のカラースケール）が占有している。**
条件付き書式は直接指定の背景色を上書きして表示するため、**背景に塗っても画面には出ない**。
2026-08-12 から 2026-09-08 まで、値下げ=赤・値上げ=水色の背景を書き込み続けていたが
**一度も表示されていなかった**（見えていた赤・青は個数の多寡を表すグラデーション）。

文字色ならカラースケール（背景のみを塗る）と共存し、個数の多寡と価格変動を同時に読める。

**塗る前に対象セルの書式をリセットする**（`_reset_price_change_style`）。挿入した列は隣接列から
書式を継承するため、リセットしないと価格が動いていない ASIN に前日の色が残る。`format` は
`userEnteredFormat` にマージするだけで消せないので、`repeatCell` + `fields` を使う。
**`fields` は `backgroundColor` と `textFormat.foregroundColor` / `foregroundColorStyle` /
`bold` だけを列挙する**（`userEnteredFormat.textFormat` ごと消すとフォントサイズまで戻る）。

前日のノートが無い日（実行が落ちた日）は比較せず色を付けない。欠測を「変化なし」と描き分けられなくなるため。

過去列の塗り直しは `recolor_price_changes.py`（`--dry-run` あり）。ノートに残った価格だけで
再現できるので、当時の価格を取り直す必要はない。**変化なしのセルは触らない** — 当日列を毎回
書き直す `write_prices` と違い、過去列は手で色を付けている可能性がある。

> **Web アプリへの移行を前提にした仕様書:**
> [`docs/superpowers/specs/2026-09-07-sales-daily-sheet-spec.md`](docs/superpowers/specs/2026-09-07-sales-daily-sheet-spec.md)
> — グリッド構造・列カタログ・データの出どころ・色の意味・API の制約・過去の事故を
> シートを見なくても再現できる粒度でまとめてある。**シートの構造を変えたらここも直す。**

## 「売上/日」は ASIN 1件につき6行（v0.44.0〜）

各 ASIN 行の直下に**ラベル行が4本**並ぶ。A列は空、商品名列にラベル名が入り、
背景色は**行全体ではなく A列〜商品名列のみ**を薄いグレーで塗る。

| # | ラベル | 中身 | 書くのは | 色 |
|---|---|---|---|---|
| 0 | （ASIN行） | その日の売上個数。ノートに価格 | `main.py daily` / `today` | 背景=うすい赤→白→青（中央値が白）／文字=値下げ赤・値上げ青 |
| 1 | `営業利益` | **数式** `=IF(粗利益="","",粗利益-N(広告費))` | 粗利益を書くたびに入れ直す | 赤字=濃い赤／0円=白→濃い青（商品ごと） |
| 2 | `広告経由` | 広告経由の売上個数 | `main.py ads` | うすい赤→白→青（中央値が白） |
| 3 | `粗利益` | 売上 −（販売手数料 + FBA手数料 + 原価）× 個数 | `main.py daily` / `today` / `finances` | 黄=見積 / 白=確定 |
| 4 | `広告費` | その日の広告費 | `main.py ads` | — |
| 5 | `順位（カテゴリ名）` | カテゴリ内の順位 | `main.py ranks` | うすい青←最上位・最下位→うすい赤 |

**#2〜#5 は行グループで折りたたむ**（`apply_row_groups.py`）。たたんだ状態でも
ASIN行（売上個数）と営業利益が並んで見えるよう、営業利益を先頭に置いている。

**ブロックの起点は必ず先頭ラベル（営業利益）から取る。** 「広告経由の1つ上が
ASIN行」と決め打つと、ラベルを並べ替えたときに別の行を掴む。実際に行の高さで
これをやり、**72件の ASIN 行を 10px に潰した**。並べ替えは `reorder_label_rows.py`
（行ごと移動するので中身と書式は失われない）。

**行1〜3は予約行。** 行1と行4が日付、**行2が総売上**、**行3が営業利益の合計**（`SUMIF` で
商品名列を引く）。いずれも千円表示。データ取得時刻は日付列ではなく「目標販売数」列の行2。

**ラベル行は ASIN 行の半分の高さにする**（`apply_row_heights.py`。`insert_label_rows.py`
が挿入後に自動で実行する）。挿入した行は ASIN 行と同じ高さで入るため、縮めないと
1商品が縦に長くなりすぎる。

**ラベル行は背景（うすい灰色）だけでなく文字色も黒で固定する。** 挿入した行は直前の
商品行から文字色を受け継ぐため、手で黄色を付けた商品ブロックでは灰色の背景に黄色の
文字が乗って読めなくなる（実際に `順位（メンズファッションヘッドバンド）` など
13行がそうなっていた）。`foregroundColorStyle` まで書くこと。`foregroundColor` だけだと
スタイル側に残った黄色が勝つ。既存行の塗り直しは `insert_label_rows.py --repaint`
（`--dry-run` あり。ラベル行を A列〜商品名列まで塗り直す。冪等）。

2026-09-06 時点で **77商品 × 5行 + 見出し8行 = 393行**。

- **営業利益は値ではなく数式で持つ。** 粗利益と広告費のどちらが後から更新されても
  追従し、書き込む順序に依存しない。広告費が空の日は `N()` が 0 として扱う
- **日付列が作られた直後は数式が無い。** `write_gross_profit` /
  `write_actual_gross_profit` が粗利益を書くたびに入れ直す。既存列の埋め直しは
  `backfill_operating_profit.py`
- **グラデーションは悪い=赤・良い=青でそろえ、行の種類は色味の濃さで見分ける。**
  個数はうすい赤→青、営業利益は濃い赤→青
- **個数は直近30日の中央値を白に置く**（2026-09-08〜）。ふだんより売れた日が青、
  売れなかった日が赤になる。MIN〜MAX の2点だと「その商品にとって普通の日」が
  色から読めず、たまたま突出した1日があると他の全日が白側に潰れていた。
  暦の1ヶ月ではなく30日固定にしているのは、月初に数日ぶんしか残らないと
  その数日の出来不出来で白の位置が飛ぶため
- **中央値が0の行は midpoint を置かず、0を白にした白→青の2点にする。** midpoint に0を
  置くと minpoint（MIN=0）とぶつかって赤が一切出ない。2026-09-08 時点で ASIN行の
  **23/74** が該当する（大半は在庫切れ・廃番で30日間まったく売れていない）
- **ASIN行と広告経由行は別ルールにする**（2026-09-08〜）。以前は1つの規則に両方の
  範囲を入れていたが、広告経由は総売上より必ず小さいため、総売上の中央値を白に
  すると広告経由行がまるごと赤に沈む。それぞれ自分の行の中央値を白にする
- **個数も営業利益も商品ごとのスケール。** 商品ごとに桁が違い（日次200個 と 数個）、
  1つに載せると規模の小さい商品がすべて同じ色になる。営業利益は 2026-09-08 まで
  全商品で1つのスケール（1000円=白〜1万円=最も濃い青）だったが、それだと規模の
  小さい商品が好調な日でも白のままで、その商品の中での良し悪しが読めなかった
- **営業利益は 0円 = 白、その商品の最大益 = 濃い青。** MIN を白にすると「最も薄利な日」が
  白になり、黒字か赤字かが色から読めない。赤字の日はグラデーションではなく
  `booleanRule`（`NUMBER_LESS 0`）が濃い赤 + 白太字で塗る（規則の順序で勝たせている）
- **粗利益にはグラデーションを掛けない。** 条件付き書式は直接指定の背景色を上書きする
  ため、掛けると「黄色=見積 / 白=確定」が見えなくなる
- **掛け直しは `main.py daily`（毎日 1:00）の末尾で自動実行する**（2026-09-08〜）。
  中央値は日々動くので、手で `apply_gradients.py` を叩く運用では実行した日の
  中央値のまま古びる。単発で掛け直したいときは従来どおり `apply_gradients.py`
- **自分が入れた規則だけを消す。** 以前は全件削除していたため、シートに元からあった
  条件付き書式を巻き込んで消し、下地の直接指定が露出した（S〜X列・AM列）
- **新しい ASIN を足したら掛け直しが要る。** 規則は行ごとに作るため、追加された行には
  規則が無い。日次ジョブが翌日に拾うが、その日のうちに色を付けたいなら手で実行する
  （行グループの折りたたみ `apply_row_groups.py` も同じ）

- **ラベルの順序と本数は3箇所で一致していなければならない。** `label_rows.ROW_LABELS_IN_ORDER`、
  `insert_label_rows.py`（既存行の移行）、そして**別リポジトリの**
...
- **粗利益は `write_prices` の後に書く。** `UnitCostReader` は原価列のヘッダー名で列を引くため、
  ...
  巻き添えで失われる
- 数値書式は粗利益・広告費とも `#,##0.0,`（1,240 → `1.2`）。**単位の文字は付けない**

## 粗利益の実測（v0.31.0〜）

`main.py finances`（毎日 3:00、`com.automation.download-amazon-data-finances`）が
**直近30日**の粗利益を実測手数料で上書きし、見積との差を「手数料乖離」シートへ出す。

### 実測は約7日遅れて出てくる（窓を30日にしている理由）

`financialEvents` は**精算期間（settlement）単位で確定する**ため、出荷しても
すぐには返ってこない。2026-09-06 に取得したとき、**8/30 以降の実測は丸ごと0**だった
（4商品すべて同じ形）。

さらに、注文日から数日遅れて出荷される分の手数料は次の精算に入る。8/23 に注文された
149個は**全て出荷済み**（レポートの `item-status` で確認、キャンセルは0）なのに、
10日経っても2個分が未計上だった。古い日ほど充足率は高いが 100% には届かない。

| 8/23 | 8/27 | 8/28 | 8/29 | 8/30以降 |
|---|---|---|---|---|
| 99% | 91% | 71% | 22% | **0%** |

**14日窓ではこれが確定する前に対象から外れ、永久に埋まらない。** 30日あれば各日に
精算2回分の猶予ができ、古い半分は完全に確定して白になる。
...
- **未出荷個数は 0 と販売個数の間に収める。** 返金は `quantity` を負にするため、
  素朴な引き算だと販売個数より多い未出荷が出る（実測が返金だけの日は負になる）
- **`orderMetrics` の個数は注文レポートの行数合計と完全に一致する。** 実測との差は
  キャンセルではない（実測期間中のキャンセルは0件だった）
...
**「手数料乖離」シート** — 毎回上書き。**FBA手数料と販売手数料を分けて**出す。

| 手数料 | 価格連動 | 差が出たときの意味 |
|---|---|---|
| FBA手数料 | **しない** | **シートの列が古い** |
| 販売手数料 | **する** | セール中なら当然下がる。列が古いとは限らない |

合算すると区別できない。実例（`B0F37S31GY`、95個）は実測 FBA **252円**/個・
`Commission` **37.2円**/個・`ShippingChargeback` 6.9円/個 に対し、シートは
FBA 318円・販売手数料 189円。見積の189円は「1,149円 × 15% × 1.1」だが、実測の37円は
「667円 × 5%」に一致する。**低価格帯の手数料率が適用され、FBA手数料も同時に下がっている。**
`ShippingChargeback` のようにシートに対応する列が無い手数料もある。

**粗利益セルのノート** — 上書き前の見積を `見積 3,900 / 実測 3,750` の形で残す。
乖離シートは直近30日しか持たないので、日ごとの履歴はここに残る。
...
- **`PostedBefore` は「2分前」より新しいと 400 になる。** 窓の開始から5分前までを取る
- **注文日は `getOrders` から取ってはいけない。** レート制限が **1分あたり1リクエスト**
  （バースト20）で、14日分の約100ページに100分かかる（実際に 429 で落ちた）。
  Reports API の `GET_FLAT_FILE_ALL_ORDERS_DATA_BY_ORDER_DATE_GENERAL` なら
  1リクエスト + ポーリングで済む（実測で1日1,372行を約20秒、圧縮なし、
  `purchase-date` は既に `+09:00`）。**日本のフラットファイルは cp932**
...
`main.py ranks`（毎日 22:00、`com.automation.category-rank`）が SP-API カタログの
`salesRanks` を取り、`順位（カテゴリ名）` 行へ書く。

- `classificationRanks`（例: ジュエリー収納 60位）を優先し、無ければ
  `displayGroupRanks`（例: ホーム＆キッチン 19888位）で代用する
- **カテゴリは一度記録したものを追い続ける。** 商品によっては複数カテゴリに順位が付くため、
  毎回「一番良い順位」を選ぶと日によってカテゴリが入れ替わり、時系列が意味を失う。
  引き継ぎ元は**ラベル名**（`順位（ジュエリー収納）`）。既にカテゴリがあればそのカテゴリの
  順位を探し、無いときだけ最上位を採る
- **順位が付いていない日は空欄のまま。** 0 を書くと「1位より下」に見える
- **日付列は作らない。** 列を作るのは `main.py daily` の責務で、無い日はスキップする
- **カタログが1件も返らなければ例外を投げてシートに触らない**（`EmptyCatalogResultError`）。
  取得できていないだけの状態で順位を消すと、実際に圏外へ落ちたのか区別できなくなる

### ラベル判定は順位だけ前方一致

順位行のラベルにはカテゴリ名が入る。`bind_label_rows` と `insert_label_rows.py` の
突き合わせは `matches_label` を通すこと。**完全一致のままだとカテゴリ名付きの順位行を
「無い」と判定し、`insert_label_rows.py` の実行のたびに1本ずつ増える。**

2026-09-09 に独立プロジェクト `marketar/category-rank` から統合した。同じシートの同じ行を
扱う処理を2箇所に分ける理由が無く、統合すれば `label_rows` を import できてラベル判定を
再実装せずに済む。旧「カテゴリランキング」シート（gid `533865782`）は履歴を売上/日 へ
移してから削除した（8/24〜9/8 の16日分・1136セル）。
...
- **既存行**にラベル行を足すのは `insert_label_rows.py`（冪等。`--dry-run` あり）。
  期待するラベル列と既存行を先頭から突き合わせるので、**途中への挿入**にも対応する
