...
**ASIN の個数書き込みは行2・行3を明示的に避ける（`row not in (TOTAL_AMOUNT_ROW, FETCH_TIME_ROW)`、v0.19.1〜）。**
`get_asin_list()` は A列が10文字の文字列というだけで ASIN 行を判定しており、**ASIN が5行目以降にある
保証はコード上どこにも無い**（実運用でそうなっているだけの慣習）。仮に ASIN がこれらの予約行と同じ行に
来ても、ガードにより個数が上書きされることはない。このガードがあるため、`requests` に行2・行4・ASIN行の
どれを先に積むかという**順序には依存しない**（以前は行2の書き込みを ASIN ループより前に置くことで
辻褄を合わせていたが、ガード導入により不要になった）。

**`write_prices` にはこのガードが無い。** 価格列（自社価格）は日付列とは別の列のため直接の事故は
起きていないが、ASIN行が仮に1〜4行目と重なった場合の挙動は保証されていない。
