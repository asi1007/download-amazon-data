...
| # | ラベル | 中身 | 書くのは | 色 |
|---|---|---|---|---|
| 0 | （ASIN行） | その日の売上個数。ノートに価格 | `main.py daily` / `today` | 背景=うすい赤→青／文字=値下げ赤・値上げ青 |
| 1 | `営業利益` | **数式** `=IF(粗利益="","",粗利益-N(広告費))` | 粗利益を書くたびに入れ直す | 赤字=濃い赤／0円=白→濃い青（商品ごと） |
| 2 | `広告経由` | 広告経由の売上個数 | `main.py ads` | うすい赤→青 |
...
**行1〜3は予約行。** 行1と行4が日付、**行2が営業利益の合計**（`SUMIF` で商品名列を引く。
データ取得時刻は同じセルのノート）、行3が総売上。いずれも千円表示。
...
- 掛け直しは `apply_gradients.py`。**自分が入れた規則だけを消す。** 以前は全件削除して
  いたため、シートに元からあった条件付き書式を巻き込んで消し、下地の直接指定が
  露出した（S〜X列・AM列）
