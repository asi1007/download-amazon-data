...
| # | ラベル | 中身 | 書くのは | 色 |
|---|---|---|---|---|
| 0 | （ASIN行） | その日の売上個数。ノートに価格 | `main.py daily` / `today` | 赤→青 |
| 1 | `広告経由` | 広告経由の売上個数 | `main.py ads` | 赤→青 |
| 2 | `営業利益` | **数式** `=IF(粗利益="","",粗利益-N(広告費))` | 粗利益を書くたびに入れ直す | 白→グレー |
...
- **グラデーションは商品ごとに独立したスケール。** 全商品を1つのスケールに載せると、
  日次200個売れる商品以外がすべて同じ色になって読めない。個数の2行（売上個数と
  広告経由）は同じスケールに載せるので、広告経由の割合が色の濃さで分かる。
  掛け直しは `apply_gradients.py`（既存の規則を消してから入れ直す）
